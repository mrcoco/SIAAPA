...
class DX_Auth
{
	...
	function _init()
	{
		...
		$this->reset_password_failed_view = $this->ci->config->item('DX_reset_password_failed_view');
                                    ...
                                    $this->header_view = $this->ci->config->item('header_view');
                                    $this->sidebar = $this->ci->config->item('sidebar');
                                    $this->notice = $this->ci->config->item('notice');
                                    $this->stats = $this->ci->config->item('stats');
                                    $this->page_index_head = $this->ci->config->item('page_index_head');
                                    $this->page_index_foot = $this->ci->config->item('page_index_foot');
                                    $this->footer_view = $this->ci->config->item('footer_view');
	}
	...
	*/
	...
	function _set_session($data)
	{
		...
		$role_data = $this->_get_role_data($data->role_id);
                ...
                $this->ci->load->model('dx_auth/users', 'users');
                $emp = $this->ci->appmodel->siapa_saya($data->id);
                $grp = $this->ci->appmodel->jenis_group($data->id);
                $menu = $this->ci->appmodel->menu_atas($data->id);
		// Set session data array
		$user = array(
                    'DX_user_id'                => $data->id,
                    'DX_username'               => $data->username,
                    'DX_email'                  => $emp['email'],
                    'DX_employee_id'            => $emp['id'],
                    'DX_nama_saya'              => $emp['fullname'],
                    'DX_foto_saya'              => $emp['foto'],
                    'DX_group_id'               => $grp['id'],
                    'DX_group_saya'             => $grp['groupname'],
                    'DX_role_id'                => $data->role_id,
                    'DX_menu_atas'              => $menu['menu'],
	...
	// Get user id $query = $this->users->get_user_by_id($value);
	...
	{
		return $this->ci->session->userdata('DX_employee_id');
	}
        
        function get_activity_note()
	{
		return $this->ci->session->userdata('DX_activity_note');
	}
        
        function get_employee_foto()
	{
		return $this->ci->session->userdata('DX_foto_saya');
	}
                    ...
                    function email_saya()
	{
        ...
        function nama_saya()
	{            
		return $this->ci->session->userdata('DX_nama_saya');
	}
        
        function groupid_saya()
	{            
		return $this->ci->session->userdata('DX_group_id');
	}
        
        function group_saya()
	{            
		return $this->ci->session->userdata('DX_group_saya');
	}
	...
	function _auto_cookie($data)
	{
		...
		set_cookie($cookie);
                ...
                function menu_saya()
	{            
		return $this->ci->session->userdata('DX_menu_atas');
	}
