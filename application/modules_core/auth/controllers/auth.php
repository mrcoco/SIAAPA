		...
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			else
			{
				...
				{						
					...
					{
						// Create catpcha						
						//$this->dx_auth->captcha();  // masih error
						// Set view data to show captcha on view file
						//$data['show_captcha'] = true;  // masih error
					}
...
//			$this->load->view($this->dx_auth->logged_in_view, $data);
                    redirect($this->own_link.'dashboard');
                    //redirect(modUri().'dashboard/tes123');
		}
	}
	...
	function logout()
	{
		...
		$data['auth_message'] = '<span>You have been logged out.</span> ';	
                $data['auth_message'] .= '<a href="'.base_url().'login" title="Back to Login">  <span class="btn  icomoon-icon-home"></span></a>'."\n";
        ...
        function timeout()
	{
		...
		$data['auth_message'] = '<span>You have been Time Out Page.</span> ';	
                $data['auth_message'] .= '<a href="'.base_url().'login" title="Back to Login">  <span class="btn  icomoon-icon-home"></span></a>'."\n";
		...
		{			
			...
			if ($val->run() AND $this->dx_auth->change_password($val->set_value('old_password'), $val->set_value('new_password')))
			{
				$data['auth_message'] = 'Your password has successfully been changed.';
                                                                           $data['notibar'] = 'msgsuccess';
				$this->_v('general', $data);
			}
			else
			{
				//$this->load->view($this->dx_auth->change_password_view);
                                                                            $this->_v('change_password_form');
			}
		}
		else
		{
